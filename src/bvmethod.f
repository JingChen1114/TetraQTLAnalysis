...
C     Date                Programmer
C     17/10/2017          Jing Chen
      subroutine bvmethod(n,mloci,ftype,mtype,rec,o,trait,kmeans1,
     /           fqtl_max,mqtl_max,BIC_min,ProfileLODs,GeneticDistance,
     /           gDistance_max,u_max,sigma_max)
      implicit double precision(a-h,o-z)
     
      parameter(mxind=500,mxloci=400,max_g=48*48,nqtype=92)
      ...
      call parent_qtl_genotype_b(fqtype,mqtype)       
      i_s1=1
      ilocus=0
      rr=0.0d0
      do im=1,nqtype
        ...
        call qtl_distribution_b(fqtl,mqtl,fre_qtl,nqtl)
        kqtl=0
        do k=1,5
          if(nqtl(k)>0)then
            kqtl=kqtl+1
        ...
        do i=2,mloci
          ...
          do while(rec1<r)
            ...
            re(ilocus(im))=rec1+rr(im)-2.0d0*rec1*rr(im)
            ge_distance(ilocus(im))=-50.0d0*log(1.0d0-2.0d0*
     /                              re(ilocus(im)))    
            do j=1,n
              do k=1,8  
              ...
              call  posszygote_b(mloci,imark,f_type,m_type,rec,r,rec1
...
C           EM ALGORITHM FOR INTERVAL MAPPING    
            call emalgorithm_b(fqtl,mqtl,n,trait,g_o,q_g,n_ig,kqtl,
     /      kmeans,u,sigma,slikelihood,sLOD)
          ...
          rr(im)=rec(i)+rr(im)-2.0d0*rec(i)*rr(im)
	  end do  
      end do    
	i_s=1
      do im=1,nqtype
        ...
        do i=1,ilocus(im)
          ...
          icheck=0
	    call check_b(fqtl,mqtl,u,icheck)
	    if(icheck==1)then
	      if(i_s==1)then
	        BIC_min=BIC_t(im,i)
	        im_max=im
...
C     ***************************************************************
      subroutine posszygote_b(mloci,imark,mp1,mp2,rec,r,rec1,flankmark,
     ...
     /                      ftype,mtype)
      ...
      do i=1,3 
        do j=1,3
          ig=0
          call pairing(i,mp1,pair11,pair12)   
          call pairing(j,mp2,pair21,pair22)   
          do i1=1,4 
            do j1=1,4
              call possgamete_b(pair11,pair12,i1,mg1,n_mode1)
              call possgamete_b(pair21,pair22,j1,mg2,n_mode2)
              do m=1,n_mode1
                do n=1,n_mode2
     ...
     /                                   zyg_p(2,zygote(3,k))+1
                  ...
                  if(sum.eq.0.0d0)then    
                    call proba_g_o_b(i1,r,g_o1)  
                    call proba_g_o_b(j1,r,g_o2)
                    call proba_g_o_multilocus_b(rec,o_ind,i,j,i1,j1,
     /                   m,n,imark,mloci,ftype,mtype,pro_l,pro_r)
                    if((pro_r.ne.0.0d0).and.(pro_l.ne.0.0d0))then
                      ...
                      call proba_q_g_b(i1,m,pair11,pair12,r,rec1,mq1,
     /                     iq1,q_g1)  
                      call proba_q_g_b(j1,n,pair21,pair22,r,rec1,mq2,
     /                     iq2,q_g2) 
                      do k1=1,iq1
                        do k2=1,iq2
...
C     ****************************************************************
      subroutine pairing(ip,mp,pair1,pair2)
...
C     ****************************************************************
      subroutine possgamete_b(pair1,pair2,mode_i,mg,n_mode)
...
C     ****************************************************************
      subroutine proba_g_o_b(ii,r,g_o)
...
C     ****************************************************************      
      subroutine proba_g_o_multilocus_b(rec,o_ind,i,j,i1,j1,m,n,
     /           imark,mloci,ftype,mtype,pro_l,pro_r)
      ...
      call ge_interval_b(i1,m,i_ga_f1,i_ga_f2)   
      call ge_interval_b(j1,n,i_ga_m1,i_ga_m2)   
      zy_matrix_l1=0.0d0
      zy_matrix_r1=0.0d0
     ...
     /                        zy_matrix_r1(k)
	    ...
	    call check_ph_b(i,j,ii,zy_matrix_r2,ftype,mtype,o_ind,
     /         zy_matrix_r1)
	  end do
	  pro_r=0.0d0
	  do k=1,16
...
C     ***************************THE LEFT PART************************
     ...
     /                        zy_matrix_l1(k)
          ...
          call check_ph_b(i,j,ii,zy_matrix_l2,ftype,mtype,o_ind,
     /                   zy_matrix_l1)
        end do
        pro_l=0.0d0
	  do k=1,16
...
C     ****************************************************************
      subroutine ge_interval_b(ii,im,i_gamete1,i_gamete2)
      ...
      if(ii==1)then  
        do i=1,2
          do j=1,2
            m=m+1
            if(m==im)then
              call num_gamete_b(i,j,i_gamete1) 
              call num_gamete_b(i,j,i_gamete2)
            end if
          end do
        end do
          ...
          do j=1,2
            do k=1,2
              if(j.ne.k)then
                ...
                if(m==im)then
                  call num_gamete_b(i,j,i_gamete1)
                  call num_gamete_b(i,k,i_gamete2)
                end if
              end if
            end do
          end do
          ...
          do j=1,2
            do k=1,2
              if(i.ne.k)then
                ...
                if(m==im)then
                  call num_gamete_b(i,j,i_gamete1)
                  call num_gamete_b(k,j,i_gamete2)
                end if
              end if
            end do
          end do
              ...
              do l=1,2
                if(i.ne.k)then
                  if(j.ne.l)then
                    ...
                    if(m==im)then
                      call num_gamete_b(i,j,i_gamete1)
                      call num_gamete_b(k,l,i_gamete2)
                    end if
                  end if
                end if 
              end do
...
C     ****************************************************************
      subroutine num_gamete_b(i1,i2,num_g)
...
C     **************************************************************
      subroutine proba_q_g_b(ii,mm,pair1,pair2,r,r1,mq,q,q_g)
...
C     ************************************************************
      subroutine qtl_distribution_b(fqtl,mqtl,fre_qtl,nqtl)
      ...
      call gametes_b(g1,fqtl,fre1) 
      call gametes_b(g2,mqtl,fre2)
...
C     ***************************************************************
      subroutine gametes_b(g,qtl,fre)
...
C     *********************************************/****************
      subroutine emalgorithm_b(fqtl,mqtl,n,trait,g_o,q_g,n_ig,kqtl,
     /                         kmeans,u,sigma,slikelihood,sLOD)
...
C     ****************************************************************
      subroutine check_ph_b(i,j,m,zy_matri,fp,mp,o_ind,genotype)
      ...
      do i1=1,4
        do j1=1,4
          call off_b(m,i,i1,fp,gamete1)
          call off_b(m,j,j1,mp,gamete2)
          do k=1,2
            k1=k+2
            zygote(k)=gamete1(k)
            zygote(k1)=gamete2(k)
...
C     ******************************************************
      subroutine off_b(m,i,i1,parent,gamete)
...
C     ***********************************************************
      subroutine tra_matrix_b(r,trans)
...
C     ***********************************************************
      subroutine parent_qtl_genotype_b(fqtype,mqtype)
      ...
      do i=1,4
        do j=i+1,5
          if(i.eq.1)then
            if(j.ne.5)then
              call iqgenotype_b(i,p1,iqtl1)
              call iqgenotype_b(j,p2,iqtl2)
              do k=1,iqtl1
                do l=1,iqtl2
                  do m=1,4
...
C     *****************************************************
      subroutine iqgenotype_b(i,p,iqtl)
...
C     ********************************************************
      subroutine check_b(fqtl,mqtl,u,icheck)
      ...
      call number_parent_b(fqtl,ip1)
      call number_parent_b(mqtl,ip2)
...
C     ***********************************************************
      subroutine number_parent_b(pqtl,ip)
